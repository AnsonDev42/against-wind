...
class OpenMeteoProvider(BaseForecastProvider):
    ...
    def get_model_run_id(self) -> str:
        ...
        try:
            with httpx.Client(timeout=self.timeout) as client:
                response = client.get(f"{self.base_url}/forecast", params={
                    "latitude": 0,
                    "longitude": 0,
    ...
    def batch_wind(self, points: List[ForecastPoint]) -> List[WindSample]:
        ...
        for point in unique_points:
            try:
                ...
                logger.error(f"Failed to fetch wind for point {point.lat}, {point.lon}: {e}")
                continue
        
        return wind_samples
    
    def _fetch_point_wind(self, point: ForecastPoint, model_run_id: str) -> List[WindSample]:
    ...
    def _parse_response(self, data: dict, point: ForecastPoint, model_run_id: str) -> List[WindSample]:
        ...
        for i, time_str in enumerate(times):
            ...
            time_diff = abs((time_utc - point.time_utc).total_seconds())
