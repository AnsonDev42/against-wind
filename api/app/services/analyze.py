...
from api.app.domain.models import (
    AnalysisRequest, Route, ForecastResult, SegmentWind, AnalysisSummary,
    ProgressEvent, PartialResultEvent, CompleteResultEvent, ErrorEvent,
    ForecastPoint, WindClass
)
...
class AnalysisService:
    ...
    async def analyze_route_stream(self, request: AnalysisRequest) -> AsyncGenerator[dict, None]:
        ...
        try:
            ...
            forecast_points = self._generate_forecast_points(route_points, request.depart_time)
            ...
            if not wind_samples:
                raise ValueError("No wind data available for the requested time and location")
            ...
            # Step 7: Complete
            yield CompleteResultEvent(data={
                "result": result.dict(),
    ...
    def _generate_forecast_points(self, route_points: List[RoutePoint], 
                                depart_time: datetime) -> List[ForecastPoint]:
    ...
    async def _process_wind_segments(self, route_points: List[RoutePoint],
                                   wind_samples: List, depart_time: datetime) -> List[SegmentWind]:
        ...
        for i, point in enumerate(route_points):
            ...
            relevant_samples = [
                sample for sample in wind_samples
            ...
            wind_speed, wind_direction = self.wind_interpolator.wind_components_to_speed_direction(
    ...
    def _generate_summary(self, segments: List[SegmentWind]) -> AnalysisSummary:
        ...
        if not segments:
            return AnalysisSummary(
                result_id="",
                head_pct=0, tail_pct=0, cross_pct=0,
                longest_head_km=0
            )
    ...
    async def _load_route_points(self, route_id: str) -> Optional[List[RoutePoint]]:
        ...
        return [
            RoutePoint(lat=51.5074, lon=-0.1278, distance_m=0, bearing_deg=90),
            RoutePoint(lat=51.5074, lon=-0.1200, distance_m=1000, bearing_deg=90),
            RoutePoint(lat=51.5074, lon=-0.1100, distance_m=2000, bearing_deg=90),
        ]
    ...
    async def _store_analysis_result(self, request: AnalysisRequest,
                                   segments: List[SegmentWind],
                                   summary: AnalysisSummary) -> ForecastResult:
